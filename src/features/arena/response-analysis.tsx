...
export function ResponseAnalysis({ responses }: { responses: readonly ResponseState[] }) {
  const result = useMemo(() => {
    const complete = responses.filter((response) => response.status === "COMPLETE" && response.text);
    if (complete.length < 2) return null;
    const scores = complete.slice(1).map((response) => overlap(complete[0].text, response.text));
    const agreement = Math.round(scores.reduce((sum, score) => sum + score, 0) / scores.length);
    return { agreement, label: agreement >= 55 ? "Mostly aligned" : agreement >= 25 ? "Mixed perspectives" : "Distinct perspectives" };
  }, [responses]);
  ...
  return (
    <aside className="border-border bg-muted/40 flex items-center justify-between gap-4 border-t px-4 py-3 text-sm">
      <div>
        ...
        <p className="text-muted-foreground text-xs">Based on shared key ideas across completed answers.</p>
      </div>
